...
class MyTonCore():
	...
	def GetValidatorStatus(self):
		...
		try:
			...
			local.add_log(f"GetValidatorStatus result: {result}", "debug")
			validatorStatus["unixtime"] = int(parse(result, "unixtime", '\n'))
			local.add_log(f"GetValidatorStatus unixtime: {validatorStatus['unixtime']}", "debug")
			validatorStatus["masterchainblocktime"] = int(parse(result, "masterchainblocktime", '\n'))
			local.add_log(f"GetValidatorStatus masterchainblocktime: {validatorStatus['masterchainblocktime']}", "debug")
			validatorStatus["stateserializermasterchainseqno"] = int(parse(result, "stateserializermasterchainseqno", '\n'))
			local.add_log(f"GetValidatorStatus stateserializermasterchainseqno: {validatorStatus['stateserializermasterchainseqno']}", "debug")
			validatorStatus["shardclientmasterchainseqno"] = int(parse(result, "shardclientmasterchainseqno", '\n'))
			local.add_log(f"GetValidatorStatus shardclientmasterchainseqno: {validatorStatus['shardclientmasterchainseqno']}", "debug")
			buff = parse(result, "masterchainblock", '\n')
			local.add_log(f"GetValidatorStatus masterchainblock: {buff}", "debug")
			validatorStatus["masterchainblock"] = self.GVS_GetItemFromBuff(buff)
			local.add_log(f"GetValidatorStatus masterchainblock: {validatorStatus['masterchainblock']}", "debug")
			buff = parse(result, "gcmasterchainblock", '\n')
			local.add_log(f"GetValidatorStatus gcmasterchainblock: {buff}", "debug")
			validatorStatus["gcmasterchainblock"] = self.GVS_GetItemFromBuff(buff)
			local.add_log(f"GetValidatorStatus gcmasterchainblock: {validatorStatus['gcmasterchainblock']}", "debug")
			buff = parse(result, "keymasterchainblock", '\n')
			local.add_log(f"GetValidatorStatus keymasterchainblock: {buff}", "debug")
			validatorStatus["keymasterchainblock"] = self.GVS_GetItemFromBuff(buff)
			local.add_log(f"GetValidatorStatus keymasterchainblock: {validatorStatus['keymasterchainblock']}", "debug")
			buff = parse(result, "rotatemasterchainblock", '\n')
			local.add_log(f"GetValidatorStatus rotatemasterchainblock: {buff}", "debug")
			validatorStatus["rotatemasterchainblock"] = self.GVS_GetItemFromBuff(buff)
			local.add_log(f"GetValidatorStatus rotatemasterchainblock: {validatorStatus['rotatemasterchainblock']}", "debug")
			validatorStatus["transNum"] = local.buffer.get("transNum", -1)
			local.add_log(f"GetValidatorStatus transNum: {validatorStatus['transNum']}", "debug")
			validatorStatus["blocksNum"] = local.buffer.get("blocksNum", -1)
			local.add_log(f"GetValidatorStatus blocksNum: {validatorStatus['blocksNum']}", "debug")
			validatorStatus["masterBlocksNum"] = local.buffer.get("masterBlocksNum", -1)
			local.add_log(f"GetValidatorStatus masterBlocksNum: {validatorStatus['masterBlocksNum']}", "debug")
	...
	def MoveCoins(self, wallet, dest, coins, **kwargs):
		...
		local.add_log("wallet.version: " + wallet.version, "debug")
		if "v1" in wallet.version:
			fiftScript = "wallet.fif"
